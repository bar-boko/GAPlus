...
def select_above (data, minValue) -> tuple:  # (idx, values)
    ...
    result_idx, result_vals = np.zeros(np.shape(a_idx), dtype = np.int32), np.zeros(np.shape(a_vals), dtype = np.int32)
    current = np.zeros(1, dtype = np.int32)
...
def full_select_above (a:tuple, virtual_places:list, data:dict, minValue:float, toJoin:bool = False) -> (
    ...
    a_array, a_valsPic = a
    physical_places, places_valsPic = p_get_virtual_valsPic(a_valsPic, virtual_places), p_make_valsPic(virtual_places,
                                                                                                       np.shape(
                                                                                                           a_valsPic))
